...
const Signup = () => {
  ...
  const handleSignup = () => {
    ...
    localStorage.setItem('ec_auth', JSON.stringify({ email, name, role }));
    navigate('/');
  };
    ...
    <div className="min-h-screen bg-background text-foreground flex items-center justify-center p-6">
      <div className="w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
        ...
        <Card className="edu-card-shadow overflow-hidden">
          <div className="px-6 pt-5 pb-3 bg-edu-blue text-white text-center font-semibold">SIGN UP</div>
          <CardContent className="space-y-5 pt-6">
            ...
            <Button className="w-full bg-edu-blue hover:bg-edu-blue-dark" onClick={handleSignup}>Create account</Button>
            <p className="text-sm text-muted-foreground text-center">
              Already have an account? <Link to="/login" className="text-edu-blue">Log in</Link>
            </p>
          </CardContent>
        </Card>
      </div>
